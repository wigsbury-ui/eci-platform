...
export function canAccessPath(pathname: string, role?: string | null): boolean {
  if (pathname.startsWith('/investor')) return !!role && INVESTOR_ROLES.includes(role as UserRole)
  if (pathname.startsWith('/school')) return !!role && SCHOOL_ROLES.includes(role as UserRole)
