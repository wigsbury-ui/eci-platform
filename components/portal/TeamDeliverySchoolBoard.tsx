...
export default function TeamDeliverySchoolBoard({
  ...
  const evidence = selected
    ? demoMode
      ? demoEvidence(selected.id)
      : []
    : []
  const reviews = selected
    ? demoMode
      ? demoReviews(selected.id)
      : []
    : []

  const activateGroup1 = async () => {
    if (demoMode) return
    const res = await fetch('/api/team/delivery/activate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ schoolId: school.id, group: 1 }),
    })
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        {!demoMode && promises.length === 0 && (
          <button
            type="button"
            onClick={activateGroup1}
            className="rounded-lg bg-[#C8A84B] px-4 py-2 text-sm font-jost font-semibold text-[#2D1654] hover:bg-[#d4b55c]"
          >
            Activate Group 1 promises
          </button>
        )}
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody>
            {sorted.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center font-jost text-sm text-gray-500">
                  No promises yet. Activate Group 1 from the framework browser or use the button above.
                </td>
              </tr>
            )}
            {sorted.map(p => (
